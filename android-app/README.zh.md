...
| 改完配置要重启 | 开 Termux 敲命令 | 设置 → 通用设置 → 底部「重启 DSH 服务」（二次确认） |
...
**已主动结案**：剩下唯一的路子是去小米开发者平台**按包名申请开通**，侧载 App 没有这个身份，为了一个
状态指示也不值得走。到那一步之前的东西都已经落地并实测过了。
...
2. **握手端点**：`127.0.0.1:3099`，只在点击图标后存活几十秒，且要求
   ...
   **交接完成后端点立即退出**，不常驻。
3. **两处会重启服务**：「重新登录」（启动页）和设置页 → 通用设置底部的
   **「重启 DSH 服务」**。两者都会打断正在进行的对话，所以都要你亲手点。
   启动页那个只在 App 明确拿不到 token（服务在跑但日志被清）时出现；设置页那个常驻，
   方便改完配置后立刻生效。设置页是 Web 内容、自己停不了 Termux 进程，所以由
   `MainActivity` 通过 `addJavascriptInterface` 暴露 `window.dshNative.restartServer()`，
   接到和「重新登录」同一条 `beginBoot(true)` 上；Web 侧那半由 `android-fix.mjs` 注入，
   且只在桥存在时渲染（桌面浏览器看到的是原样的设置页）。
